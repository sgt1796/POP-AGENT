...
If the download returns HTML instead of a PDF, treat it as a landing page and use the returned `final_url`, landing page title, `content_preview`, `pdf_link_candidates`, or `saved_landing_page_path` to recover the real document or a readable preview before searching elsewhere.
